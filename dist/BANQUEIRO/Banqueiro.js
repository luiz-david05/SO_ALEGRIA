...
class Processo {
    _id;
    _recursosNecessarios;
    _recursosAlocados;
    constructor(_id, _recursosNecessarios, _recursosAlocados) {
        this._id = _id;
        this._recursosNecessarios = _recursosNecessarios;
        this._recursosAlocados = _recursosAlocados;
    }
    get id() {
        return this._id;
    }
...
function removerProcesso(processos, processo) {
    return processos.filter(p => p !== processo);
}
function verifica_recursos_pra_processo(recursos_alocados, recursos_necessarios, recursos_disponiveis) {
    const quantidade_de_recursos = recursos_alocados.length;
    let recursos_faltantes;
...
function adiciona_recursos_utilizados_ao_disponivel(recursos_alocados, recursos_disponiveis) {
    const quantidade_de_recursos = recursos_alocados.length;
    for (let i = 0; i < quantidade_de_recursos; i++) {
        recursos_disponiveis[i] += recursos_alocados[i];
    }
}
async function algoritmo_do_banqueiro(processos, recursos_disponiveis) {
    let copyProcessos = [...processos];
